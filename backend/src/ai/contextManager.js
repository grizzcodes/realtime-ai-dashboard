...
class ContextManager {
  ...
  detectPatterns(event, tasks) {
    const pattern = {
      ...
      avgUrgency: tasks.reduce((sum, t) => sum + t.urgency, 0) / tasks.length,
  ...
  getPeopleContext(keyPeople = []) {
    return keyPeople.map(person => {
      ...
      const avgUrgency = personData.urgencyPattern.reduce((a, b) => a + b, 0) / personData.urgencyPattern.length;
  ...
  getUrgencyContext(task) {
    ...
    const avgUrgency = recentUrgencies.reduce((a, b) => a + b, 0) / recentUrgencies.length || 3;
  ...
  generateProjectSuggestions() {
    ...
    this.context.activeProjects.forEach((project, projectName) => {
      const daysSinceActivity = Math.floor((new Date() - project.lastActivity) / (1000 * 60 * 60 * 24));
      
      if (daysSinceActivity > 5 && project.priority >= 3) {
        suggestions.push({
          type: 'project_check_in',
          priority: 'medium',
          message: `${projectName} needs attention`,
    ...
    return suggestions.slice(0, 2); // Top 2 project suggestions
  }
