...
const simpleTests = {
  notion: async () => {
    ...
    return result.success ? 
      { success: true, message: 'Notion API connected successfully!' } : 
      result;
  },
  slack: () => ({ 
...
Object.keys(simpleTests).forEach(service => {
  app.get(`/api/test/${service}`, async (req, res) => {
    const result = typeof simpleTests[service] === 'function' ? 
      await simpleTests[service]() : simpleTests[service];
    
    if (result.success && !result.message) {
      result.message = `${service} configuration found`;
    }
